...
MODEL_XML_PATH = os.path.join('baxter', 'pick_and_place_obstacle.xml')


class BaxterPickAndPlaceObstacleEnv(baxter_env.BaxterEnv, utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        initial_qpos = {
            ...
            'robot0:left_s0': -0.708302176857093,  # -----baxter
            'robot0:left_s1': -0.9522185740798705,  # -----baxter
            'robot0:left_e0': -0.553383569229663,  # -----baxter
            'robot0:left_e1': 1.4285196087182916,  # -----baxter
            'robot0:left_w0': 0.29375732088005674,  # -----baxter
            'robot0:left_w1': 1.230636087081073,  # -----baxter
            'robot0:left_w2': -0.7136845615636888,  # -----baxter
            ...
            'object0:joint': [1.65, 0.259027, -0.039, 1., 0., 0., 0.],
            # 'object0:joint': [0., 0., 0., 0., 0., 0., 0.],

        }
        baxter_env.BaxterEnv.__init__(
            self, MODEL_XML_PATH, has_object=True, block_gripper=False, n_substeps=20,
            # TODO: How n_substeps slow down the step in viewer?
    ...
    def _sample_goal(self):
        if self.has_object:
            goal = self.initial_gripper_xpos[:3] + self.target_offset + self.np_random.uniform(-self.target_range,
                                                                                               self.target_range,
                                                                                               size=3)
            goal[1] = self.np_random.uniform(low=0.23, high=0.4 )
            goal[0] = self.np_random.uniform(low=0.5, high=0.7)
    ...
    def _reset_sim(self):
        ...
        if self.has_object:
            ...
            object_xpos[1] = self.np_random.uniform(low=0, high=0.12)
            object_xpos[0] = self.np_random.uniform(low=0.5, high=0.7)
