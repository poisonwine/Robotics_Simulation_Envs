...
class CatchEnv(robot_env_m.RobotEnvM):
    ...
    def _get_obs(self):
        ...
        front_frame = self.sim.render(width=300, height=300, camera_name='fixed', depth=False)
        beside_frame = beside_frame[::-1,:,:]
        front_frame = front_frame[::-1, :, :]
    ...
    def get_image_desired_goal(self):
        if self.has_object:
            ...
            self.sim.data.set_joint_qpos('object0:joint', object_qpos)
            self.sim.forward()
            goal_image = self.sim.render(width=300, height=300, camera_name='gripper_camera_rgb', depth=False)
